...
CREATE TABLE `t_goods` (
    ...
    `warning_num` int NOT NULL DEFAULT 0 COMMENT '库存预警阈值',
...
CREATE TABLE `t_order` (
    ...
    `logistics_trace` VARCHAR(2000) NULL COMMENT '物流轨迹JSON',
    `shipper_code` VARCHAR(20) COMMENT '快递公司编码（如 SF/STO/YTO）',
    `logistic_code` VARCHAR(50) COMMENT '快递单号',
    `refund_type` TINYINT DEFAULT 0 COMMENT '退款类型：0=无 1=仅退款 2=退货退款',
    `refund_status` TINYINT DEFAULT 0 COMMENT '退款状态：0=无 1=待审核 2=审核通过 3=已退款 4=已拒绝 5=退款失败',
    `refund_reason` VARCHAR(255) DEFAULT NULL COMMENT '退款原因',
    `refund_apply_time` DATETIME DEFAULT NULL COMMENT '退款申请时间',
    `refund_amount` DECIMAL(10,2) DEFAULT NULL COMMENT '退款金额',
    `refund_order_no` VARCHAR(32) DEFAULT NULL COMMENT '售后工单号（退款单号）',
     PRIMARY KEY (`id`),
...
CREATE TABLE `t_order_log` (
    ...
    `refund_order_no` varchar(64) DEFAULT NULL COMMENT '售后工单号',
